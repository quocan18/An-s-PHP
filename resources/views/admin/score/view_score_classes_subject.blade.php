...
@section('body')
<form action="{{ route('view_score_classes_subjects') }}" method="get">
	{{csrf_field()}}
Select classes
...
<table class="table">
	...
	@foreach($array_students as $students)
		<tr>
			...
			<td>
				<input type="number" min="0" max="10" name="array_score[{{$students->students_id}}]" value="{{$array_score[$students->students_id]}}">
			</td>
		</tr>
	@endforeach
</table>
